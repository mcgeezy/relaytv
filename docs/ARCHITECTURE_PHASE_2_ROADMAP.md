...
open. PR #21 merged on 2026-07-03 and this branch was rebased onto `main`
(dropping the now-squashed Phase 1 commits) the same day.
...
Status: complete (rebased onto `main` 2026-07-03 after PR #21's squash
merge: `git rebase --onto origin/main <phase-1 head>`; content parity
verified — the only tree delta was the release 0.4.0 version bump the old
branch base predated)
...
| 2026-07-03 | [#22](https://github.com/mcgeezy/relaytv/pull/22) | `main` | PR #21 merged; rebased this branch onto `main` past the squash merge (content parity verified) and opened the final Phase 2 PR. | `ruff check app tests`; `PYTHONPATH=app pytest -q` (268 passed, post-rebase); `git diff --check` | Merge PR #22 to complete Phase 2. |
| 2026-07-03 | [#22](https://github.com/mcgeezy/relaytv/pull/22) | `main` | CI now runs `ruff check app tests` and the full `pytest -q` suite (previously only `tests/test_smoke.py`, so none of the Phase 1/2 guardrail tests ran in CI). Running the suite in a CI-like clean venv surfaced a FastAPI 0.139 incompatibility in the route-inventory helper (lazy router includes are no longer flattened into `app.routes`); fixed by recursing through included routers, verified on fastapi 0.118 and 0.139. | clean-venv `pip install -e '.[dev]'` + `pytest -q` (268 passed, fastapi 0.139); system `PYTHONPATH=app pytest -q` (268 passed, fastapi 0.118); `ruff check app tests`; `git diff --check` | Merge PR #22 to complete Phase 2. |
